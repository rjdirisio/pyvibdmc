...
def oh_dists(cds,idxs):
    oh1 = xp.linalg.norm(cds[:, idxs[0]] - cds[:, idxs[1]], axis=1)
    return oh1


def gmat():
    return inv_mh / r1_eq ** 2 + inv_mh / r2_eq ** 2 + inv_mo * \
           (1 / r1_eq ** 2 + 1 / r2_eq ** 2 - 2 * xp.cos(theta_eq) / (r1_eq * r2_eq))
...
def trial_wavefunction(cds, ex_args, ret_pdt=True):
    ...
    cds = xp.asarray(cds)
    psi = xp.zeros((len(dists) + len(angs), len(cds)))
    for i in range(len(dists)):
        ohs = oh_dists(cds, dists[i])
        psi[i] = xp.interp(ohs,gridpts,wfn)
...
def first_deriv(ohs,hohs):
    ...
    dpsi = xp.zeros((len(ohs) + len(hohs), len(ohs[0])))
...
def sec_deriv(ohs,hohs):
    ...
    sdpsi = xp.zeros((len(ohs) + len(hohs), len(ohs[0])))
    for i in range(len(ohs)):
...
def dpsi_dx(cds, ex_args):
    ...
    dists = ex_args['dists'] # list of lists
    nw = len(dists) // 2
    grouped_dists = xp.reshape(xp.asarray(dists),(nw,2,2)) # num_waters, num_ohs (always 2) , atms 0 and 1
    angs = ex_args['angs']

    cds = xp.asarray(cds)

    ohz = xp.asarray([oh_dists(cds,dist) for dist in dists])
    angz = xp.asarray([bond_angle(cds,ang_idx=ang) for ang in angs])
    ...
    dint_dx = xp.zeros((len(angs) + len(dists),) + cds.shape)
    d2int_dx2 = xp.zeros((len(angs) + len(dists),) + cds.shape)

    for num, hoh in enumerate(angs):
        ohs = grouped_dists[num]
        cos_theta = xp.cos(angz[num]) 
        dr_dxs = xp.stack([crh.dr_dx(oh) for oh in ohs])
        d2r_dx2s = xp.stack([crh.d2r_dx2(oh, dr_dx=dr_dxs[num]) for num, oh in enumerate(ohs)])
