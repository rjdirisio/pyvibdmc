...
class DMC:
    def __init__(self,
        ...
        self._dwSaveStep = self._wfnSaveStep+self.DwSteps
    ...
    def birthOrDeath_vec(self,vref, Desc):
        ...
        if self.weighting == 'discrete':
            ...
            deathMask = np.logical_or((1 - np.exp(-1. * (self._walkerV - vref) * self.deltaT)) < randNums, self._walkerV < vref)
            ...
            birthMask = np.logical_and((np.exp(-1. * (self._walkerV - vref) * self.deltaT) - 1) > randNums, self._walkerV < vref)
    ...
    def run(self):
        self.propagate()
        np.save(self.outputFolder+"/"+self.simName+"_energies.npy",Constants.convert(self._vrefAr,"wavenumbers",to_AU=False))
        if self.weighting == 'discrete':
            np.save(self.outputFolder + "/" + self.simName + "_population" + ".npy",self._popAr)
